...
module "proxy_authentication" {
  ...
  [for n in each.value.network.networks : n.ipv4_address if n.name == "br1"][0],
  [for n in each.value.network.networks : n.ipv4_address if n.name == "br0"][0],
  each.value.network.networks[0].ipv4_address
